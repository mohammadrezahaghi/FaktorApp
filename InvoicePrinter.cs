...
namespace FactorApp.UI.Helpers
{
    public class InvoicePrinter
    {
        ...
        private FlowDocument CreateDesign()
        {
            ...
            // مارجین کل صفحه (10 برای جا شدن در A5)
            Grid invoiceLayer = new Grid { Margin = new Thickness(20) };
            ...
            // >>>>> سایدبار (راست)
            StackPanel sidebar = new StackPanel { Margin = new Thickness(5, 0, 0, 0) };

            Image logoImage = new Image { Width = 70, Height = 70, Stretch = Stretch.Uniform, HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0, 0, 0, 5) };
            ...
            sidebar.Children.Add(new TextBlock { Text = $"تاریخ: {ToPersian(DateUtils.ToShamsi(_invoice.Date))}", FontSize = 10, Foreground = shayanBlue, HorizontalAlignment = HorizontalAlignment.Center });
            ...
            // شماره فاکتور
            bool isValidNumber = !string.IsNullOrWhiteSpace(_invoice.InvoiceNumber) && _invoice.InvoiceNumber != "0" && !_invoice.InvoiceNumber.ToLower().Contains("draft");
            if (isValidNumber)
            {
            ...
            int rowIndex = 1;
            int itemCounter = 1;
            var rowHeight = new GridLength(25); // ارتفاع کم برای جا شدن
            ...
            footerGrid.ColumnDefinitions.Add(new ColumnDefinition()); // سمت راست (خریدار)
            footerGrid.ColumnDefinitions.Add(new ColumnDefinition()); // سمت چپ (فروشنده)
            ...
            footerGrid.Children.Add(new TextBlock
            {
                ...
                // افزایش مارجین راست برای فاصله گرفتن از مرکز
                Margin = new Thickness(0, 0, 60, 0),
                FontSize = 11,
                FontWeight = FontWeights.Bold,
                VerticalAlignment = VerticalAlignment.Center // <<<<<< این خط باعث می‌شود خریدار وسط‌چین عمودی شود
            });

            // 2. سمت فروشنده (طراحی جدید: متن راست، عکس چپ)
            // از یک گرید داخلی استفاده می‌کنیم تا متن و عکس کنار هم باشند
            Grid sellerContainer = new Grid
            {
                HorizontalAlignment = HorizontalAlignment.Left,
                // افزایش مارجین چپ برای فاصله گرفتن از مرکز
                Margin = new Thickness(40, 0, 0, 0),
                Height = 80,
                VerticalAlignment = VerticalAlignment.Top
            };

            // ستون 0: متن (چون راست‌چین است، سمت راست می‌افتد)
            // ستون 1: عکس‌ها (سمت چپ می‌افتد)
            sellerContainer.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });
            sellerContainer.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });

            // الف) متن فروشنده
            TextBlock sellerText = new TextBlock
            {
                Text = "فـروشـنـده",
                FontSize = 10,
                FontWeight = FontWeights.Bold,
                VerticalAlignment = VerticalAlignment.Center,
                Margin = new Thickness(0, 0, 15, 0) // فاصله متن از عکس‌ها
            };
            Grid.SetColumn(sellerText, 0);
            sellerContainer.Children.Add(sellerText);

            // ب) گروه عکس‌ها (مهر و امضا)
            ...
            Grid.SetColumn(imageGroup, 1);
            sellerContainer.Children.Add(imageGroup);

            // افزودن کانتینر فروشنده به فوتر
            Grid.SetColumn(sellerContainer, 1);
            footerGrid.Children.Add(sellerContainer);
